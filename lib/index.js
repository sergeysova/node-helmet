...
class Tag {
   ...
   * @param {string} type mime type
   */
  static style(type = 'text/css') {
    return new Tag('style').attr('type', type)
  }
}
...
assignStatic(Tag, tag, ['div', 'h', 'link', 'title', 'script', 'style'])
...
class Helmet extends Html {
  ...
  /**
   * Append <style ...$props>$content</style>
   * @param {string} content
   * @param {Object} props
   */
  inlineStylesheet(content, props) {
    const style = Tag.style((props && props.type) || 'text/css')
    applyAttrs(style, props)
    style.setChildren(content)
    this.head(style)
    return this
  }
